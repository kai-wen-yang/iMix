...
def cifar10(regim='train', root=Path.db_root_dir('cifar10'), transform=None):
    return CIFAR10(root=root, train=regim=='train', download=True, transform=transform)

def cifar100(regim='train', root=Path.db_root_dir('cifar100'), transform=None):
    return CIFAR100(root=root, train=regim=='train', download=True, transform=transform)
